...
def cornersHeuristic(state, problem):
    ...
    not_visited = []
    for u in corners:
        if u not in Visited_Corners:
            not_visited.append(u)
    
    def mindist(x,y,list):
        if len(list) == 0:
            return 0
        distlist=[]
        for nodes in list:
            newlist = list.copy()
            newlist.remove(nodes)
            distlist.append(abs(x-nodes[0])+abs(y-nodes[1])+mindist(nodes[0],nodes[1],newlist))
        return min(distlist)
    newnewlist = not_visited.copy()
    h_sum=mindist(x,y,newnewlist)
