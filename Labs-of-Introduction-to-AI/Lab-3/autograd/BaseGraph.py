...
class Graph(List):
    ...
    def backward(self, grad=1.0, debug=False):
        ...
        for i in range(len(self)):
            ans = self[-i-1].backward(ans,debug)
        if debug:
    ...
    def optimstep(self, lr, wd1, wd2):
        ...
        for nd in self:
            if len(nd.params) != 0:
                nd.grad[0] += wd1*np.sign(nd.params[0])
                nd.grad[0] += 2*wd2*nd.params[0]
               
                nd.params[0] -= lr*nd.grad[0]*(1-count/1000)
            if len(nd.params) > 1:
                nd.params[1] -= lr*nd.grad[1]*(1-count/1000)
        return 0
