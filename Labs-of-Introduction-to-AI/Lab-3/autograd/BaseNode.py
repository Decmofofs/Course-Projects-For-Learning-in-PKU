...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        Q=np.array([X[i,:,:]@W_q for i in range(X.shape[0])])
        K=np.array([X[i,:,:]@W_k for i in range(X.shape[0])])
        V=np.array([X[i,:,:]@W_v for i in range(X.shape[0])])
        
        score =np.array([Q[i,:,:] @ K[i,:,:].T for i in range(X.shape[0])],dtype=float)  / np.sqrt(K.shape[-1])
...
class Linear(Node):
    ...
    def cal(self, X):
        ...
        # 与lab2相同
            ...
            result = X@self.params[0]+self.params[1]
            self.cache.append((X,result))
            return result
            raise NotImplementedError
    ...
    def backcal(self, grad):
        ...
        # 与lab2相同
        X=self.cache[-1][0]
        gX=grad@self.params[0].T
        
        gW=X.T@grad
        
        gB=np.sum(grad,axis=0)
        self.grad=[gW,gB]
        return gX
        raise NotImplementedError
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        ...
        gX=grad@self.params[0].T
        gW=np.transpose(X,(0,2,1))@grad
        gB=np.sum(grad,axis=0)
        gW2 = np.average(gW,axis=0)
        gB2 = np.average(gB,axis=0)
        self.grad=[gW2,gB2]
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        expX = np.exp(X)
        softmax = expX / expX.sum(axis=self.dim, keepdims=True)
        log_softmax = np.log(softmax+1e-7)
        self.cache.append((softmax,log_softmax))
        return log_softmax
        raise NotImplementedError

    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        softmax_X = self.cache[-1][0]
        return grad-np.multiply(grad.sum(axis=self.dim,keepdims=True),softmax_X)
        raise NotImplementedError
