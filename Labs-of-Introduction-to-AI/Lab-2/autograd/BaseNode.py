...
#第一个需要实现的部分
    ...
    def backcal(self, grad):
        ...
        def derivative_sigmoid(a):
            return a*(1-a)
        return np.multiply(grad,np.vectorize(derivative_sigmoid)(self.cache[-1]))
        raise NotImplementedError        
...
#第四个需要实现的部分
    ...
    def backcal(self, grad):
        ...
        new_X = np.take_along_axis(X,np.expand_dims(y,axis=-1),axis=-1)
        np.put_along_axis(result, np.expand_dims(y, axis=-1), np.vectorize(func)(new_X), axis=-1)
        return grad*result
        raise NotImplementedError